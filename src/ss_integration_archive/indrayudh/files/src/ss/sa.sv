module sa #(
    ...
    input wire en,
    input wire [ARRAY_WIDTH-1:0] load, //LSB -> left col, MSB -> right col

    input wire [ARRAY_HEIGHT*DATA_WIDTH-1:0] i_data, //LSB -> top pe, MSB -> bottom pe
    output wire [ARRAY_WIDTH*ACC_WIDTH-1:0] o_data  //LSB -> left pe, MSB -> right pe
);

genvar h, w;

//Delay the incoming load signal based on the column number. 
//Left column receives immediately. Right column receives ARRAY_WIDTH-1 delayed.
...
//Delay incoming data from left into correct skew.
//top pe(LSB) has no delay. bottom pe(MSB) has ARRAY_HEIGHT-1 delay.
